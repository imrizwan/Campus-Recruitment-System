...
const validateLanguageInput = require("../validation/language");
const validateProjectStu = require("../validation/projectstu");
...
class AuthController {
    ...
    verifyUser(req, res) {
        User.findOneAndUpdate({ _id: req.params.id }, { $set: { isVerified: true } }, { new: true })
    ...
    uploadPicture(req, res) {
        ...
        upload(req, res, function (err) {
            ...
            if (err) {
                ...
                // });
                Profile.findOneAndUpdate({ user: req.user.id }, { $set: { url: req.file.secure_url } }, { new: true })
                    .then(() => {
    ...
    uploadPictureById(req, res) {
        ...
        upload(req, res, function (err) {
            ...
            if (err) {
                ...
                //     console.log("Cloudinary: ", result)
                // });
                Profile.findOneAndUpdate({ user: req.params.id }, { $set: { url: req.file.secure_url } }, { new: true })
                    .then(() => {
                    return res.status(200).json({ success: true });
                })
                    .catch(err => {
            ...
            if (isEmpty(data)) {
                Verify.findOne({ user: req.user.id })
                    ...
                    Verify.findOne({ user: req.body.companyid })
                        ...
                        .catch(err => console.log("err from applyforVaccancy4 Company", err));
                    data
                        .save()
    ...
    deleteEducation(req, res) {
        Profile.updateOne({ user: req.user.id }, { $pull: { education: { _id: req.query.id } } }, { multi: true })
            .then(data => res.status(200).json({ success: 200 }))
            .catch(err => console.log("err from delete education", err));
    }
    // delete language
    deleteLanguage(req, res) {
        Profile.updateOne({ user: req.user.id }, { $pull: { language: { _id: req.query.id } } }, { multi: true })
            .then(data => res.status(200).json({ success: 200 }))
            .catch(err => console.log("err from delete language", err));
    }
    // deleteExperience
    deleteExperience(req, res) {
        Profile.updateOne({ user: req.user.id }, { $pull: { experience: { _id: req.query.id } } }, { multi: true })
            .then(data => res.status(200).json({ success: 200 }))
            .catch(err => console.log("err from delete experience", err));
    }
    // deleteActivities
    deleteActivities(req, res) {
        Profile.updateOne({ user: req.user.id }, { $pull: { activities: { _id: req.query.id } } }, { multi: true })
            .then(data => res.status(200).json({ success: 200 }))
            .catch(err => console.log("err from delete deleteActivities", err));
    }
    // delete project
    deleteProject(req, res) {
        Profile.updateOne({ user: req.user.id }, { $pull: { projects: { _id: req.query.id } } }, { multi: true })
            .then(data => res.status(200).json({ success: 200 }))
    ...
    createprofile(req, res) {
        ...
        if (req.body.mail)
            profileFields.mail = req.body.mail;
        ...
        if (req.body.semester)
            profileFields.semester = req.body.semester;
    ...
    getAllProfileCreated(req, res) {
        Verify.find()
            .then((data) => res.status(200).json(data))
            .catch((err) => console.log("err from getAllProfileCreated", err));
    }
    recommend(req, res) {
        Verify.findOneAndUpdate({ user: req.params.id }, {
            $set: {
        ...
        Profile.findOne({ user: req.params.id })
            ...
            if (profile) {
                User.findOneAndUpdate({ _id: req.params.id }, {
                    ...
                    .catch(err => console.log("Error from create Profile 1: ", err));
                // Update
                Profile.findOneAndUpdate({ user: req.params.id }, { $set: profileFields }, { new: true })
                    .then(profile => res.json(profile))
                    .catch(err => console.log("Error from create Profile 1: ", err));
            }
            else {
