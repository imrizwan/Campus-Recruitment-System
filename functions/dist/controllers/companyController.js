...
class CompanyAuthController {
    createcompanyprofile(req, res) {
        ...
        CompanyProfile.findOne({ user: req.user.id }).then(profile => {
            if (profile) {
                // Update
                CompanyProfile.findOneAndUpdate({ user: req.user.id }, { $set: profileFields }, { new: true }).then(profile => res.json(profile))
    ...
    updateVaccancy(req, res) {
        ...
        CompanyProfile.updateOne({ vaccancy: { $elemMatch: { _id: req.body.id } } }, { $set: {
    ...
    deleteVaccany(req, res) {
        ...
        CompanyProfile.updateOne({ user: req.user.id }, { $pull: { "vaccancy": { _id: req.query.vaccancyid } } }, { multi: true })
